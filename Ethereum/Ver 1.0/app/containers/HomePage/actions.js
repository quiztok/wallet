...
export function generatePasswordCancel() {
  //console.log("취소")
  return {
    type: GENERATE_PASSWORD_CANCEL,
  };
}
...
export function generateKeystoreSuccess(keystore, tokenList) {
  ...
  localStorage.setItem("addr",addresses);
...
export function lockWallet() {
  message.success('지갑이 성공적으로 잠겼습니다.');
  return {
    type: LOCK_WALLET,
  };
}
...
export function unlockWalletSuccess(password) {
  ...
  message.success('지갑 잠금 해제가 되었습니다.');
  return {
...
export function closeWallet() {
  message.success('지갑 닫기가 성공하였습니다.');
  return {
    type: CLOSE_WALLET,
  };
}
...
export function userPasswordChange(error) {
  console.log(error);
}