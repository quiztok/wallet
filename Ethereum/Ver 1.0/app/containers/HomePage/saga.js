...
export function* generateWallet() {
  try {
    ...
    const password = document.getElementById("userPasswords").value;
    const extraEntropy = generateString(generatedPasswordLength);
    const seed = lightwallet.keystore.generateRandomSeed(extraEntropy);
    
    localStorage.setItem("keystore",lightwallet.keystore);
    localStorage.setItem("sd",seed);
    localStorage.setItem("ecommendPassword",generateString(generatedPasswordLength));
...
export function* restoreFromSeed() {
  try {
    ...
    if (userPassword.length < 8) {
      yield put(restoreWalletFromSeedError('비밀번호는 8자리 이상 입력하십시오.'));
      return;
    }
...
export function* genKeystore() {
  try {
    ...
    ks.passwordProvider = (callback) => {
      // const password = yield select(makeSelectPassword());
      const pw = prompt('비밀번호를 입력하십시오.', '비밀번호'); // eslint-disable-line
      callback(null, pw);
    };
...
export function* generateAddress() {
  try {
    ...
    if (!ks) {
      throw new Error('개인키 정보를 찾을 수 없습니다.');
    }

    const password = yield select(makeSelectPassword());
    if (!password) {
      // TODO: Handle password
      throw new Error('지갑 잠금');
    }
...
export function* unlockWallet() {
  try {
    ...
    if (currentPassword) {
      throw Error('지갑이 이미 잠금 해제되었습니다.');
    }

    const ks = yield select(makeSelectKeystore());
    if (!ks) {
      throw new Error('잠금 해제할 키 정보가 없습니다.');
    }
    ...
    const errorString = `지갑 장금 해제 오류 - ${err.message}`;
...
export function* loadWalletS() {
  try {
    ...
    if (existingKs) {
      throw new Error('기전의 개인키 정보가 확인되었습니다.');
    }
