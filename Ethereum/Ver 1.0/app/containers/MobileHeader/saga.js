  ...
  try {
    ...
    if (keystore) {
      ...
      if (param.sendType) {
        ...
        yield put(showSendToken2(localStorage.getItem('addr'),tokenName));
        yield put(changeTo2(param.tokenToAddress));
...
export function* confirmSendTransaction() {
  try {
    ...
    const toAddress = $("#toAddressInput").val();
    const gasPrice = yield select(makeSelectGasPrice());

    if (!web3.isAddress(fromAddress)) {
      throw new Error('받는 분의 지갑 주소를 입력하세요.');
    }

    if (amount <= 0) {
      throw new Error('이체 금액을 입력하세요.');
    }
    ...
    if (!(gasPrice > 0.1)) {
      throw new Error('수수료 가격은 최소 0.1 (Gwei) 이상  입력하셔야 합니다.');
    }

    const msg = `이체 정보가 확인되었습니다. 
