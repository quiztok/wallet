...
function AddressItem(props) {
  ...
  const ethData = data.get('eth', 'QTCON');

  const balance = ethData.get('balance') !== false ? `${ethData.get('balance').div(Ether).toString(10)} ETH ` : '0';

  const rate = exchangeRates.getIn([convertTo, 'rate']);
  const convertedBalance = (balance !== '0' && rate) ? ethData.get('balance').div(Ether).times(rate).toFixed(2).toString(10) : '';
